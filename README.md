...
| Database      | MySQL target environments, H2 local metadata fallback                              |
| Persistence   | Flyway, Spring Data JPA                                                            |
| SQL           | JSQLParser 5.3, Spring JDBC, HikariCP                                              |
| Config        | Spring external config, optional Nacos Config/Discovery                            |
...
前置条件：JDK 21 可用。默认本地 profile 使用 H2 内存元数据库，不需要 MySQL 或 Nacos。
...
本地空环境 jar 启动、MySQL 元数据库、Nacos、反向代理/TLS 和内网访问限制请从
