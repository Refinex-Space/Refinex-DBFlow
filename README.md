...
项目当前形态是 Spring Boot WebMVC 单体应用，管理端以 Spring MVC + Thymeleaf `/admin` 作为稳定入口，同时提供
React 新后台 `/admin-next` 作为试运行入口。MCP endpoint 使用 Spring AI MCP WebMVC Server，目标数据库连接由
HikariCP 按 project/env 维护。
...
- **管理端**：`/login` 登录，`/admin` 进入稳定后台，`/admin-next` 进入 React 新后台试运行；支持用户、Token、project/env
...
| React 新后台试运行 | `http://localhost:8080/admin-next`       |
...
| 启动 React 新后台开发    | `pnpm --dir dbflow-admin dev`                                 |
| 测试 React 新后台      | `pnpm --dir dbflow-admin test`                                |
| 构建 React 新后台      | `pnpm --dir dbflow-admin build`                               |
| 打包 React 新后台进 jar | `./mvnw -Preact-admin -DskipTests package`                    |
...
+-- dbflow-admin/                       # React 新后台，开发态入口 /admin-next
