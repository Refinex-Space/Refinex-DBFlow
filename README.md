...
项目当前形态是 Spring Boot WebMVC 单体应用，管理端以 React SPA `/admin` 作为正式入口，旧 Thymeleaf
后台短期保留在 `/admin-legacy` 仅用于过渡期排障。MCP endpoint 使用 Spring AI MCP WebMVC Server，
目标数据库连接由 HikariCP 按 project/env 维护。
...
- **管理端**：`/login` 登录，`/admin` 进入 React 正式后台，`/admin-legacy` 仅用于短期排障；支持用户、Token、project/env
...
| 旧 Thymeleaf 后台 | `http://localhost:8080/admin-legacy`     |
...
+-- dbflow-admin/                       # React 管理端，正式入口 /admin
