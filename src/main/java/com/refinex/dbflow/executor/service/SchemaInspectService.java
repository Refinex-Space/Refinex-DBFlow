...
public class SchemaInspectService {
    ...
    public SchemaInspectResult inspect(SchemaInspectRequest request) {
        ...
        try (Connection connection = dataSourceRegistry.getDataSource(request.projectKey(), request.environmentKey())
            ...
            boolean truncated = SchemaMetadataMapper.isTruncated(maxItems, rawSchemas, rawTables, rawColumns,
                    rawIndexes, rawViews, rawRoutines);
            return new SchemaInspectResult(
                    ...
                    maxItems,
                    truncated,
                    SchemaMetadataMapper.bounded(rawSchemas, maxItems),
                    SchemaMetadataMapper.bounded(rawTables, maxItems),
                    SchemaMetadataMapper.bounded(rawColumns, maxItems),
                    SchemaMetadataMapper.bounded(rawIndexes, maxItems),
                    SchemaMetadataMapper.bounded(rawViews, maxItems),
                    SchemaMetadataMapper.bounded(rawRoutines, maxItems),
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                ...
                while (resultSet.next()) {
                    rows.add(SchemaMetadataMapper.database(resultSet));
                }
                return rows;
            }
        }
    ...
    private List<SchemaTableMetadata> queryTables(Connection connection, String schema, String table, int limit)
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                ...
                while (resultSet.next()) {
                    rows.add(SchemaMetadataMapper.table(resultSet));
                }
                return rows;
            }
        }
    ...
    private List<SchemaColumnMetadata> queryColumns(Connection connection, String schema, String table, int limit)
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                ...
                while (resultSet.next()) {
                    rows.add(SchemaMetadataMapper.column(resultSet));
                }
                return rows;
            }
        }
    ...
    private List<SchemaIndexMetadata> queryIndexes(Connection connection, String schema, String table, int limit)
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                ...
                while (resultSet.next()) {
                    rows.add(SchemaMetadataMapper.index(resultSet));
                }
                return rows;
            }
        }
    ...
    private List<SchemaViewMetadata> queryViews(Connection connection, String schema, String table, int limit)
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                ...
                while (resultSet.next()) {
                    rows.add(SchemaMetadataMapper.view(resultSet));
                }
                return rows;
            }
        }
    ...
    private List<SchemaRoutineMetadata> queryRoutines(Connection connection, String schema, int limit)
        ...
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                ...
                while (resultSet.next()) {
                    rows.add(SchemaMetadataMapper.routine(resultSet));
                }
                return rows;
            }
        }
