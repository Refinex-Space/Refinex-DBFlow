...
项目当前形态是 Spring Boot WebMVC 单体应用，管理端以 `dbflow-admin/` 中的 React SPA 作为正式前端，入口仍是
`/login` 和 `/admin`。MCP endpoint 使用 Spring AI MCP WebMVC Server，目标数据库连接由 HikariCP 按 project/env
维护；MCP 不复用管理端 session，每次请求仍必须携带 Bearer Token。
...
- **管理端**：`/login` 登录，`/admin` 进入 React 正式后台；支持用户、Token、project/env
  授权、配置查看、危险策略、审计、健康状态。
...
| Admin Frontend | React, Vite, TypeScript, shadcn/ui, Tailwind CSS, TanStack Router/Query/Table      |
...
| 启动 React 管理端开发    | `pnpm --dir dbflow-admin dev`                                 |
| 测试 React 管理端      | `pnpm --dir dbflow-admin test`                                |
| 构建 React 管理端      | `pnpm --dir dbflow-admin build`                               |
| 打包 React 管理端进 jar | `./mvnw -Preact-admin -DskipTests package`                    |
...
- MCP endpoint 不读取或复用管理端 session，必须使用 `Authorization: Bearer <DBFlow Token>`。
- dev 初始管理员凭据来自外部配置；首次登录后应立即修改密码。
...
| 产品/设计  | [后台 UI 设计系统](docs/references/admin-ui-design-system.md), [后台管理原型说明](docs/prototypes/admin/README.md), [后台 HTML 原型](docs/prototypes/admin/index.html) |
...
- [docs/runbooks/troubleshooting.md](docs/runbooks/troubleshooting.md) - 启动失败、Nacos、数据库、Token、MCP、策略和 SQL
  执行故障排查。
- [docs/references/admin-ui-design-system.md](docs/references/admin-ui-design-system.md) - 后台前端视觉、布局、配色、交互和可访问性标准。
- [docs/PLANS.md](docs/PLANS.md) - Harness 执行计划索引，包含 active 和 completed plans。

React 管理端 cutover 状态：`/login` 和 `/admin` 是当前后台入口；旧 Thymeleaf 后台已删除，`/admin-legacy/**`
不再作为排障或管理入口。
...
- 后台前端 UI、布局、样式、配色和交互遵循 [后台 UI 设计系统](docs/references/admin-ui-design-system.md)。
- 设计决策写入 `docs/exec-plans/specs/`；实现计划写入 `docs/exec-plans/active/`，完成后归档到
  `docs/exec-plans/completed/`。
- `docs/PLANS.md` 是 Harness 计划索引；设计决策和执行计划分别沉淀在 `docs/exec-plans/specs/` 与
  `docs/exec-plans/completed/`。
